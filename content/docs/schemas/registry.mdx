...
**Overlay interop:** The APP.md frontmatter maps directly to the `PublishedAppMetadata` interface from `metanet-apps`. Publishing via ClawNet also broadcasts a PushDrop token to `tm_apps` (protocol `[1, 'metanet apps']`) so the app appears in any wallet's app catalog.
...
**Dual-publish pattern:** Apps published via ClawNet CLI are automatically broadcast to both:
1. The ClawNet registry (Convex-backed, with stars/downloads/attestations)
2. The `tm_apps` overlay (PushDrop tokens, discoverable via `metanet-apps`)
