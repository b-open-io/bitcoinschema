...
Output 1:  file inscription (1 sat) — refs/example.md
...
Output N:  manifest inscription (1 sat, tracked ordinal)
           Content-Type: ord-fs/json
           Content: { "SKILL.md": "{{vout:0}}", "refs/example.md": "{{vout:1}}" }
...
The manifest is an inscription with content type `ord-fs/json`. Its content is a JSON object mapping file paths to sibling output references:

```json
{
  "SKILL.md": "{{vout:0}}",
  "refs/example.md": "{{vout:1}}",
  "scripts/setup.sh": "{{vout:2}}"
}
```

After broadcast, indexers resolve `{{vout:N}}` placeholders to `ord://{txid}_{N}`. ORDFS serves the package as a directory — requesting `/content/{manifestOutpoint}/SKILL.md` returns the file content.
...
const files = [
  ...
  { path: "refs/api.md", content: apiDocs, contentType: "text/markdown" },
];
...
const manifest = {};
files.forEach((f, i) => { manifest[f.path] = `{{vout:${i}}}`; });
