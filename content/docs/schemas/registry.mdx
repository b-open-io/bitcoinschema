...
All registries share the same `registry:*` type system, `_N` relative vout references, and `ord-fs/json` manifest format. A BigBlocks component published on-chain is installable via any compatible gateway:
...
Both serve the same shadcn-compatible JSON — one from on-chain data, one from the registry server.
...
const mapFields = {
  ...
  name: "bitcoin-avatar",
  version: "1.0.0",
  description: "Bitcoin identity avatar with on-chain profile resolution",
  title: "Bitcoin Avatar",
  author: "Satchmo",
  language: "en",
  dependencies: JSON.stringify(["@radix-ui/react-avatar"]),
  registryDependencies: JSON.stringify(["button"]),
  categories: JSON.stringify(["identity", "social"]),
};
